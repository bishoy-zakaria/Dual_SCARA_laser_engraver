...
def mean_absolute_error(a,b):
    ...
    for day in range(len(a)):
        ...
        if a[day][0] !=0:
            per_err_1 = (a[day][0] - b[day][0]) / a[day][0]
        else:
            continue
        if a[day][1] !=0:
            per_err_2 = (a[day][1] - b[day][1]) / a[day][1]
        else:
            continue
  
        # Take absolute value of
        # the percentage error (APE)
        per_err_1 = abs(per_err_1)
        per_err_2 = abs(per_err_2)
        
  
        # Append it to the APE list
        APE_1.append(per_err_1)
        APE_2.append(per_err_2)
  
    # Calculate the MAPE
    MAPE = ((sum(APE_1)/len(APE_1))+(sum(APE_2)/len(APE_2)))/2
  
    # Print the MAPE value and percentage
    return round(MAPE*100, 2) 

lenth = 0.20 #upper links
lenth_2 = 0.275 #motor links
motor_distance=0.20
Data_1= [0]
Data_2= [0]
Data_3= [0]
...
def System_Simulation(file_link):
    
    global G_CO,Data,int_data,float_data,detect,sign_detect,G90_CO,G91_CO,G0_act,G1_act,e1,e2,e3,e4,progress
    ...
    e1 = format(mean_absolute_error(Data_ideal, Data_1))
    e2 = format(mean_absolute_error(Data_ideal, Data_2)) 
    e3 = format(mean_absolute_error(Data_ideal, Data_3))
    e4 = format(mean_absolute_error(Data_ideal, Data_4))
